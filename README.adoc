...
** If they were started as Docker containers, make sure you are in `spring-cloud-stream-kafka-elasticsearch` root folder and run the script below
+
[source]
----
./stop-apps.sh
...
* Stop and remove docker-compose containers, network and volumes by running the following command inside `spring-cloud-stream-kafka-elasticsearch` root folder
+
[source]
----
...
* To remove the Docker images created by this project, make sure you are in `spring-cloud-stream-kafka-elasticsearch` root folder and run the following script
+
[source]
----
